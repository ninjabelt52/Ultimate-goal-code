...
@Autonomous (name = "Blue side parking, start on right", group = "Blue")
@Disabled
public class Blue_side_park2 extends LinearOpMode {
    ...
    public void runOpMode()throws InterruptedException {
        ...
        DrivewDistanceandEncoder(7,.4,0,280);
    ...
    public void DrivewDistanceandEncoder(double range, double straight,double strafe,int distance)throws InterruptedException{
        ...
        while ((!(Distance.getDistance(DistanceUnit.INCH) < range) && !isStopRequested()) && ((backLeftWheel.getCurrentPosition() <= distance) && (backRightWheel.getCurrentPosition() <= distance))){
            angles = imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
            double currentHeading = angles.firstAngle;
